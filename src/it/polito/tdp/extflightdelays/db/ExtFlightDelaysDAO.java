...
public class ExtFlightDelaysDAO {
	...
	public List<Airport> loadAllAirports(Map<Integer,Airport> aIdMap) {
		...
		try {
			...
			while (rs.next()) {
				//Se l'oggetto non è presente nell'identity map
	...
	public List<Rotta> getRotte(Map<Integer, Airport> aIdMap, int distanzaMedia){
		String sql = "SELECT ORIGIN_AIRPORT_ID AS id1, DESTINATION_AIRPORT_ID AS id2, AVG(DISTANCE) AS avgg "+
				...
				"HAVING avgg > ? ";
				// CON LA GROUP BY COLLASSO INSIEME TUTTE LE RIGHE CHE HANNO LO 
				// STESSO AEROPORTO DI ORIGINE E DI DESTINAZIONE
		...
		try {
			...
			while (rs.next()) {
				Airport source = aIdMap.get(rs.getInt("id1"));
				Airport destination = aIdMap.get(rs.getInt("id2"));
